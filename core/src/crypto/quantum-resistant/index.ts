  ...
  if (typeof data === 'object') {
    try {
      ...
      if (error instanceof TypeError && error.message.includes('circular')) {
        ...
        issues.push(`Object serialization error: ${error instanceof Error ? error.message : 'Unknown error'}`);
      }
    }
  }
  ...
  if (typeof data === 'object') {
    if (Array.isArray(data)) {
      ...
      for (const [key, value] of Object.entries(data)) {
        if (typeof key === 'string' && key.length < 1000) { // Limit key length
...
export function convertDataSafely<T>(data: unknown, expectedType: string): T {
  ...
  if (expectedType === 'string' && typeof sanitized === 'string') {
    return sanitized as T;
  }
  
  if (expectedType === 'number' && typeof sanitized === 'number') {
    return sanitized as T;
  }
  
  if (expectedType === 'boolean' && typeof sanitized === 'boolean') {
    return sanitized as T;
  }
  
  if (expectedType === 'object' && typeof sanitized === 'object') {
    return sanitized as T;
  }
  
  if (expectedType === 'array' && Array.isArray(sanitized)) {
    return sanitized as T;
  }
  
  throw new Error(`Data type mismatch: expected ${expectedType}, got ${typeof sanitized}`);
}
