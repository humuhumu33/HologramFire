...
/**
 * Optimized Throughput Benchmark Runner for HoloMatrix
 * 
 * Implements all throughput optimizations to achieve ≥1 Gbit/s:
 * 1. Increase iterations from 10 to 200-500 to saturate lanes
 * 2. Optimize worker count to match cores/NIC queues (8-16)
 * 3. Increase batch size to 128-256 to amortize per-send overhead
 * 4. Optimize payload size to 4-8KB sweet spot (8192 bytes)
...
function getOptimalConfig() {
  ...
  const config = {
    ...
    duration: 30,        // was 10, now 30 for proper saturation
...
function printBanner(config) {
  console.log('🚀 OPTIMIZED THROUGHPUT BENCHMARK - HOLOMATRIX');
  console.log('='.repeat(60));
  console.log('📋 Quick Wins Implementation:');
  console.log(`   ✅ Duration: ${config.duration}s (was 10) - saturate lanes`);
...
async function runOptimizedBenchmark(config) {
  console.log('\n📊 Running optimized HoloMatrix benchmark...');
  console.log('='.repeat(60));
  ...
  try {
    await runCommand('node', ['node_modules/.bin/ts-node', 'src/steps/04-bench-optimized.ts', ...args]);
    console.log('✅ Optimized HoloMatrix benchmark completed successfully');
  } catch (error) {
    console.error('❌ Optimized HoloMatrix benchmark failed:', error.message);
    throw error;
  }
}
...
async function runMatrixSweep() {
  ...
  try {
    await runCommand('node', ['node_modules/.bin/ts-node', 'src/steps/04-bench-optimized.ts', '--matrix']);
...
// Run original benchmark for comparison
async function runOriginalBenchmark() {
  ...
  const args = [
    '--duration', originalConfig.duration.toString(),
    '--lanes', originalConfig.lanes.toString(),
    '--payload', originalConfig.payload.toString(),
    '--batch', originalConfig.batch.toString(),
    '--workers', originalConfig.workers.toString(),
    '--target', originalConfig.target.toString(),
    '--window', originalConfig.window.toString()
  ];
  
  try {
    await runCommand('node', ['node_modules/.bin/ts-node', 'src/steps/04-bench.ts', ...args]);
    console.log('✅ Original benchmark completed successfully');
  } catch (error) {
    console.error('❌ Original benchmark failed:', error.message);
    throw error;
  }
}
...
async function main() {
  ...
  try {
    // 1. Run original benchmark for comparison
    await runOriginalBenchmark();
    
    // 2. Run optimized benchmark with quick wins
    await runOptimizedBenchmark(config);
    
    // 3. Run matrix sweep to find optimal configuration
    await runMatrixSweep();
    
    // 4. Run stress test with high parameters
    await runStressTest();
    
    console.log('\n🎉 ALL OPTIMIZED HOLOMATRIX BENCHMARKS COMPLETED!');
    console.log('='.repeat(60));
    console.log('📊 Summary:');
    console.log('   ✅ Original benchmark completed for comparison');
    ...
    console.log('   • 1.2-1.5x from eliminated hot-path overhead');
