...
describe("G13: attestation meets targets", () => {
  it("bench → compare against targets", () => {
    ...
    const c = cp.spawnSync("ts-node", ["--transpile-only","tools/scripts/bench-compare.ts","--in=artifacts/perf/attestation.json","--targets=atlas-12288/perf/targets.json"], { encoding: "utf8", shell: true });
